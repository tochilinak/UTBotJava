                ...
                is PythonEvaluationError -> {
                    val utError = UtError(
                        "Error evaluation: ${evaluationResult.status}, ${evaluationResult.message}\n${
                            evaluationResult.stackTrace.joinToString(
                                "\n"
                            )
                        }",
                ...
                is PythonEvaluationSuccess -> {
                    ...
                    when (result) {
                        is ValidExecution -> {
                            ...
                            PythonExecutionResult(
                                result,
                                PythonFeedback(control = Control.CONTINUE, result = trieNode, typeInferenceFeedback = typeInferenceFeedback)
                            )
                        }
                        is InvalidExecution -> {
                            description.limitManager.addInvalidExecution()
    ...
    fun fuzzing(
        ...
        ServerSocket(0).use { serverSocket ->
            ...
            val pmd = PythonMethodDescription(
                ...
                Trie(Instruction::id),
                Random(0),
                TestGenerationLimitManager(ExecutionWithTimeoutMode, until),
